  ...
  <div>
    {type === 'order_processing_kk' && (
      <div>
        ...
        <p>
          Order Date: {new Date(orderData.created_at).toLocaleString()}<br />
          Preferred Delivery Date: {orderData.pickup_on}<br />
          Order Number: {orderData.id}<br />
          Link to Items for Delivery: {orderData.product.url}<br />
          Pickup Address: {orderData.logistics.from}<br />
          Delivery Address: {orderData.logistics.to}<br />
        ...
        <p>
          Best regards,<br />
          KleinKurier Team<br />
          Email: info@kleinanzeigenkurier.de<br />
    ...
    {type === 'order_processing_customer' && (
      <div>
        ...
        <p>
          Order Date: {new Date(orderData.created_at).toLocaleString()}<br />
          Order Number: {orderData.id}<br />
          Link to Item: {orderData.product.url}<br />
          Items to be delivered to {orderData.logistics.to} and from {orderData.logistics.from}
        </p>
        <p>
          We will notify you once your order has been processed. In the meantime, if you have any questions or need further assistance, feel free to contact us.
        </p>
        <p>
          Best regards,<br />
          KleinKurier Team<br />
    ...
    {type === 'order_processed_success' && (
      <div>
        ...
        <p>
          ...
          Pickup Address: {orderData.logistics.from}<br />
          Delivery Address: {orderData.logistics.to}<br />
          Delivery Date: {orderData.pickup_on}<br />
          Pickup Time: {orderData.pickup_between}<br />
    ...
    {type === 'order_processed_failure' && (
      <div>
        ...
        <ul>
          ...
          <li>Delivery area is not accessible by foot or public transportation</li>
          <li>Selected delivery time is not feasible</li>
          <li>Chosen delivery date is unavailable</li>
          <li>All delivery personnel are currently booked for the foreseeable future</li>
